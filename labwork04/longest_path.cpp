...
struct hash_map
{
    std::string key;
    int value;
    bool operator<(const hash_map &obj) const
...
int main()
{
    ...
    while (num_map > 0)
    { // parent process wait to receive result from all mapper
        int pdata;
        read(pfd[0], &pdata, 4);
        if (pdata >= 0)
        ...
        { // get end signal from a mapper
            num_map--;
        }
    }

    while ((wpid = wait(&status)) > 0)
        ; // wait untill all slave process end
